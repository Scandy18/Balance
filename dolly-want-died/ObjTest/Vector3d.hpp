...
public:
	...
	float GetModule() { return x * x + y * y + z * z; }

	Vector3d UnitVector() { Vector3d  n(x, y, z); if ((!n) == 0) return Vector3d(0,0,0); return n / !n; }

	float GetAngel(Vector3d d) { if (isZero() || d.isZero()) return 0;Vector3d  n(x, y, z); return (n | d) / ((!n)*(!d)); }
	...
	void operator /= (float t) { x = x / t;y = y / t;z = z / t; }
	//取模 !vector
	float operator ! () { float t = sqrt(GetModule());return t; }
	//点积（找不到点怎么打）v1|v2
